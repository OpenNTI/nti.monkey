...
def check_threadlocal_status(names=('transaction.ThreadTransactionManager',
    ...
    for name in names:
        ...
        if not isinstance(obj, type):
            obj = type(obj)
        logger.debug("Checking gevent status of %s=%s: %s",
...
if version_info[0] >= 1 and 'ZEO' not in sys.modules:
    ...
    # patching it anyways. So it should be safe to remove. In 1.1, it's true
    # by default.
    ...
    # patch to os.pipe (below). Sigh, so close. (With a little work, we could
    ...
    # Only import this after the patch, it allocates locks
    logger = __import__('logging').getLogger(__name__)
